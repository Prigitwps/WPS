# Arizona-4 test module for OES 1.0
# Requirements:
...
# Usage: python arizona4Regression.py [-h] [-r] [-e] [-d] [-l] [-u]
# Arguments:
...
testInfo = {   'assessmentform': 'Record Form',
    'assessmenttype': 'arizona4',
    'clientagedays': '17',
    'clientagemonths': '4',
    'clientageyears': '4',
    'clientcaseid': '',
    'clientemail': '',
    'clientfirstname': 'Arizona4-QA',
    'clientgender': 'Female',
...
argsListAssessment = {
'Examiner Information': {'Confidence interval': '95', 'Examiner name/ID': 'Arizona-4 Test Examiner'},
'Client Information': {'Grade': 'Preschool', 'Dialect': 'Received Pronunciation'}
}

argsListValidation = {}

questionList = {   'Sentence Articulation': {   'q1': ['Correct'],
                                 'q10': ['Correct'],
                                 'q11': ['Correct'],
                                 'q12': ['Incorrect'],
                                 'q13': ['Correct'],
        ...
        # if logToFile:
            # with open(os.path.join(os.path.join(str(directory), str(logName)), str(logName + (time.strftime("_%F_%T", time.gmtime())).replace(":", "-") + ".txt")), 'w', encoding='utf-8') as f:
                # with redirect_stdout(f), redirect_stderr(f):
                    ...
                    # logging.info("Specified output directory is: " + str(directory))
...
# def runTest(env="uat", headless=False, randomResponse=False, directory=None, input=None, waitTime=None):
    ...
    #logger = logging.getLogger(os.path.splitext(os.path.basename(str(__file__)))[0])
    # Trying to grab the logger directly off the Prac object...
    logger = Prac.logger
